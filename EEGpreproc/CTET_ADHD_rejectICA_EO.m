...
for nF=1:length(files)
    ...
    if redo==1 || exist([data_path filesep 'Preproc' filesep 'CIcf_ft_' file_name(1:end-4) '.mat'])==0
        ...
        load([data_path filesep 'Preproc' filesep 'Icf_ft_' file_name(1:end-4)]);
        fprintf('... processing %s\n',file_name(1:end-4));
        data_ori=data;
        
        %%% Reject bad component
        cfg = [];
        this_line=match_str(ICA_table.FileName,['Icf_ft_' file_name(1:end-4)]);
        if isempty(this_line)
            warning(sprintf('... missing %s in ICA detection\n',file_name(1:end-4)));
            continue;
        end
        these_components=ICA_table.blinks{this_line};

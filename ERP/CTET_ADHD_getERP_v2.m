...
for nF=1:length(files)
    ...
    table=readtable([save_path filesep 'CTET_ADHD_behav_SW_' SubID '_CTET.txt']);
    ...
    cfgerp3.trials = find(table.StimType==0 & table.Fz==0); %cfgerp.trials(cfgerp.trials>length(data_clean.trial))=[];
    if ~isempty(cfgerp3.trials)
        av_data_NT_noSW = ft_timelockanalysis(cfgerp3, data_clean);
        ERP_NT_noSW=av_data_NT_noSW.avg; %(:,av_data_TG.time>-0.2 & av_data_TG.time<0.8);
        all_ERP_NT_noSW(nFc,:,:)=ERP_NT_noSW;
    else
        all_ERP_NT_noSW(nFc,:,:)=nan;
    end
    
    cfgerp3.trials = find(table.StimType==0 & table.Fz==1); %cfgerp.trials(cfgerp.trials>length(data_clean.trial))=[];
    if ~isempty(cfgerp3.trials)
        av_data_NT_SW = ft_timelockanalysis(cfgerp3, data_clean);
        ERP_NT_SW=av_data_NT_SW.avg; %(:,av_data_TG.time>-0.2 & av_data_TG.time<0.8);
        all_ERP_NT_SW(nFc,:,:)=ERP_NT_SW;
    else
        all_ERP_NT_SW(nFc,:,:)=nan;
    end
    
    cfgerp3.trials = find(table.StimType==1 & table.Fz==0); %cfgerp.trials(cfgerp.trials>length(data_clean.trial))=[];
    if ~isempty(cfgerp3.trials)
        av_data_TG_noSW = ft_timelockanalysis(cfgerp3, data_clean);
        ERP_TG_noSW=av_data_TG_noSW.avg; %(:,av_data_TG.time>-0.2 & av_data_TG.time<0.8);
        all_ERP_TG_noSW(nFc,:,:)=ERP_TG_noSW;
    else
        all_ERP_TG_noSW(nFc,:,:)=nan;
    end
    
    cfgerp3.trials = find(table.StimType==1 & table.Fz==1); %cfgerp.trials(cfgerp.trials>length(data_clean.trial))=[];
    if ~isempty(cfgerp3.trials)
        av_data_TG_SW = ft_timelockanalysis(cfgerp3, data_clean);
        ERP_TG_SW=av_data_TG_SW.avg; %(:,av_data_TG.time>-0.2 & av_data_TG.time<0.8);
        all_ERP_TG_SW(nFc,:,:)=ERP_TG_SW;
    else
        all_ERP_TG_SW(nFc,:,:)=nan;
    end
...
fprintf('... unpaired t-test between groups on diff ERP between TG and NT on POz for 1.1-1.8s: p=%g, t-value=%g, df=%g\n',...
...
fprintf('... one-sample t-test with 0 for Controls on diff ERP between TG and NT on POz for 1.1-1.8s: p=%g, t-value=%g, df=%g\n',...
    pV_CTR,stat_CTR.tstat,stat_CTR.df)
[h, pV_ADHD,~,stat_ADHD]=ttest(diffTG_NT(match_str(group_PowDataEO,'ADHD')),0);
fprintf('... unpaired t-test with 0 for ADHD on diff ERP between TG and NT on POz for 1.1-1.8s: p=%g, t-value=%g, df=%g\n',...

...
%         pause;
    ...
    res_mat=[res_mat; [nF*ones(size(this_behav,1),1) this_behav this_cleanRT]];
    group_cond=[group_cond ; repmat({Group},size(this_behav,1),1)];

    for nbl=1:8
        resblock_mat=[resblock_mat; [nF nbl nanmean(this_behav(this_behav(:,3)==nbl & this_behav(:,4)==0,6)==0) nanmean(this_behav(this_behav(:,3)==nbl & this_behav(:,4)==0,6)==1) ...

...
for nF=1:length(files)
    ...
    for nE=1:64
        ...
        if ~isempty(paramSW.fixThr)
            thr_Wave_final(nFc,nE)=paramSW.fixThr;
        else
            paramSW.prticle_Thr=mean(thr_Wave_all(match_str(group_SW,'Control'),nE));
            thr_Wave_final(nFc,nE)=paramSW.prticle_Thr;
        end
        slow_Waves=[slow_Waves ; thisE_Waves(temp_p2p>thr_Wave_final(nFc,nE),:)];
    end
    save([data_path filesep 'Preproc' filesep 'relThrCTR_CIcfeblock_ft_SW_' file_name(1:end-4)],'slow_Waves','paramSW')

...
for nF=1:length(files)
   ...
   load([data_path filesep 'Preproc' filesep 'TF_' file_name(1:end-4)]);
    ...
    av_DurationEO(nFc,1) =TFdata.time(end)-1;
    av_DurationEO(nFc,2) =NaN;
    
    av_PowDataEO(nFc,1,:,:) = squeeze(10*log10(nanmean(TFdata.powspctrm,4)));
    av_PowDataEO(nFc,2,:,:) = nan;
...
tic;
   ...
   load([data_path filesep 'Preproc' filesep 'TF_' file_name(1:end-4)]);
    ...
    av_PowDataEO(thisF,2,:,:) = squeeze(10*log10(nanmean(TFdata.powspctrm,4)));
     av_DurationEO(thisF,2) =TFdata.time(end)-1;
...
%Plot topo report
temp_topo=squeeze(nanmean(nanmean(av_PowDataEO(match_str(group_PowDataEO,'Control'),2,matching_elec,TFdata.freq>4 & TFdata.freq<7),4),1));
figure;
...
for i = 1:2
    for j = 1:2
        if j==1
            temp=squeeze(nanmean(av_PowDataEO(match_str(group_PowDataEO,'Control'),i,match_str(TFdata.label,'Fz'),TFdata.freq>4 & TFdata.freq<7),4));
        datatoplot{i, j} = temp(~isnan(temp));
        else
                    temp=squeeze(nanmean(av_PowDataEO(match_str(group_PowDataEO,'ADHD'),i,match_str(TFdata.label,'Fz'),TFdata.freq>4 & TFdata.freq<7),4));
datatoplot{i, j} =  temp(~isnan(temp));
        end
    end
end
...
format_fig;
Theta_Before=squeeze(nanmean(av_PowDataEO(:,1,match_str(TFdata.label,'Fz'),TFdata.freq>4 & TFdata.freq<7),4));
Theta_After=squeeze(nanmean(av_PowDataEO(:,2,match_str(TFdata.label,'Fz'),TFdata.freq>4 & TFdata.freq<7),4));
t = table(group_PowDataEO((~isnan(Theta_After)))', Theta_Before(~isnan(Theta_After)),Theta_After(~isnan(Theta_After)),'VariableNames', {'Group', 't1', 't2'});
Time = [1 2]';
rm = fitrm( t, 't1-t2~Group', 'WithinDesign', Time);
fprintf('rmANOVA for Fz: F=%g p=%1.5f\n',ranovatbl.F(2),ranovatbl.pValue(2))

temp_topo_Fval=[];
temp_topo_pval=[];
for nE=1:size(av_PowDataEO,3)
    Theta_Before=squeeze(nanmean(av_PowDataEO(:,1,nE,TFdata.freq>4 & TFdata.freq<7),4));
    Theta_After=squeeze(nanmean(av_PowDataEO(:,2,nE,TFdata.freq>4 & TFdata.freq<7),4));
